---
name: pets
description: Maintain Nypsi's economy pets feature, including pet data, items, Prisma storage, cached functions, activation slots, the Components v2 pets command, item use, season reset, and farm/fish/hunt/mine bonuses. Use whenever adding, changing, displaying, activating, upgrading, or integrating pets.
---
...
- Supported pets are only `cow`, `beaver`, `tiger`, and `mole`.
...
- Format descriptions by replacing `{chance}` and `{bonus}`. Farm bonuses display
  `benefit * 100`; fish, hunt, and mine descriptions intentionally use the simpler
  `x% chance to ... more items` wording.
...
limited to validating `data/pets.json`.
