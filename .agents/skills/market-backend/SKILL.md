...
- Atomic creation holds the Redis item mutex, a transaction-scoped PostgreSQL advisory item lock,
  locks matching market rows in deterministic ID order with `FOR UPDATE`, and locks existing
