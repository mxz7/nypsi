...
module.exports = new ItemUse("gold_credit", async (message: Message | (NypsiCommandInteraction & CommandInteraction)) => {
  ...
  if (currentTier > GOLD_TIER) return send({ embeds: [new ErrorEmbed("your current premium tier is higher than gold")] });
  ...
  if (currentTier == GOLD_TIER) {
    ...
    return send({
      embeds: [
        new CustomEmbed(
          message.member,
          `your gold membership will now expire <t:${Math.floor(profile.expireDate.getTime() / 1000)}:R>`
        ),
      ],
    });
    ...
    return send({
      embeds: [
        new CustomEmbed(message.member, `your **gold** membership will expire <t:${dayjs().add(7, "day").unix()}:R>`),
      ],
    });
  } else {
