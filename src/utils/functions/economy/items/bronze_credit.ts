...
module.exports = new ItemUse("bronze_credit", async (message: Message | (NypsiCommandInteraction & CommandInteraction)) => {
  ...
  if (currentTier == BRONZE_TIER) {
    ...
    await setInventoryItem(message.member, "bronze_credit", inventory.find((i) => i.item === "bronze_credit").amount - 1);
    ...
    return send({
      embeds: [
        new CustomEmbed(
          message.member,
          `your bronze membership will now expire <t:${Math.floor(profile.expireDate.getTime() / 1000)}:R>`
        ),
      ],
    });
    ...
    return send({
      embeds: [
        new CustomEmbed(message.member, `your **bronze** membership will expire <t:${dayjs().add(7, "day").unix()}:R>`),
      ],
    });
  } else {
    ...
    const inventory = await getInventory(message.member, false);
    if (!inventory.find((i) => i.item === "bronze_credit") || inventory.find((i) => i.item === "bronze_credit").amount < 1) {
