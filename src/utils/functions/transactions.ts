...
export async function formatTransaction(transaction: Transaction, dest?: "discord") {
  return (
    ...
    `${dest === "discord" ? "`" : ""}${transaction.sourceId}${dest === "discord" ? "`" : ""} (${await getLastKnownUsername(transaction.sourceId, dest === "discord")}) -> ` +
    `${dest === "discord" ? "`" : ""}${transaction.targetId}${dest === "discord" ? "`" : ""} (${await getLastKnownUsername(transaction.targetId, dest === "discord")}) ` +
