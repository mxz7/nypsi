  ...
  setTimeout(
    async () => {
      ...
      if (await redis.exists(`${Constants.redis.nypsi.RICKROLL}:${message.author.id}`)) {
        ...
        addProgress(userId, "rick_roll", 1);
    ...
    for (const tag of hourlyCommandCount.keys()) {
      ...
      if (uses > 350) {
        ...
        if (uses > 400 && typeof id === "string") {
          ...
          if (dayjs().subtract(30, "seconds").valueOf() > lastCommand.getTime()) continue; // don't lock if last command was more than 5 minutes ago
