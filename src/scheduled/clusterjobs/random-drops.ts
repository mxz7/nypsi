...
import { escapeSpecialCharacters, getZeroWidth } from "../../utils/functions/string";
...
export async function startLootRain(channel: GuildTextBasedChannel, user: User) {
  ...
  await channel.send({
    embeds: [
      new CustomEmbed(
        null,
        `**${escapeSpecialCharacters(user.username)}'s loot rain is starting!!!**`,
      ).setColor(0xffffff),
    ],
  });
    ...
    if (!active) {
      channel.send({
        embeds: [
          new CustomEmbed(
            null,
            `**${escapeSpecialCharacters(user.username)}'s loot rain has ended.**`,
          ).setColor(0xffffff),
        ],
      });
      return;
