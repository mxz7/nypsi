...
import { getEmbedColor, getTier, isPremium, setEmbedColor } from "../utils/functions/premium/premium";
...
async function run(message: Message | (NypsiCommandInteraction & CommandInteraction), args: string[]) {
  ...
  if ((await getTier(message.author.id)) < 1) {
    return message.channel.send({
      embeds: [
        new ErrorEmbed("you must be atleast BRONZE tier for this command, you are BRONZE\n\nhttps://www.patreon.com/nypsi"),
      ],
    });
  }
