...
async function run(
  ...
  } else if (args[0].toLowerCase() === "migratewordle") {
    if (message.author.id !== Constants.TEKOH_ID) return;

    const oldStats = await prisma.wordleStats.findMany();

    for (const stat of oldStats) {
      for (let i = 0; i < stat.win1; i++) {
      ...
      for (let i = 0; i < stat.win3; i++) {
        await prisma.wordleGame.create({
          data: {
            time: 0,
            won: true,
            guesses: ["meow", "meow", "meow"],
            word: "meow",
            userId: stat.userId,
          },
        });
      }
      for (let i = 0; i < stat.win4; i++) {
        await prisma.wordleGame.create({
          data: {
            time: 0,
            won: true,
            guesses: ["meow", "meow", "meow", "meow"],
            word: "meow",
            userId: stat.userId,
          },
        });
      }
      for (let i = 0; i < stat.win5; i++) {
        await prisma.wordleGame.create({
          data: {
            time: 0,
            won: true,
            guesses: ["meow", "meow", "meow", "meow", "meow"],
            word: "meow",
            userId: stat.userId,
          },
        });
      }
      for (let i = 0; i < stat.win6; i++) {
        await prisma.wordleGame.create({
          data: {
            time: 0,
            won: true,
            guesses: ["meow", "meow", "meow", "meow", "meow", "meow"],
            word: "meow",
            userId: stat.userId,
          },
        });
      }
      for (let i = 0; i < stat.lose; i++) {
        await prisma.wordleGame.create({
          data: {
            time: 0,
            won: false,
