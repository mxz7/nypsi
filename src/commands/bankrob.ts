...
async function run(message: Message | (NypsiCommandInteraction & CommandInteraction), args: string[]) {
    ...
    if (args[0] == "status") {
        ...
        const embed = new CustomEmbed(message.member, bankList).setHeader("current bank balances");
    ...
    const embed = new CustomEmbed(message.member, "robbing " + bank + "..").setHeader(
        "bank robbery",
        message.author.avatarURL()
    );

    const embed2 = new CustomEmbed(message.member, "robbing " + bank + "..").setHeader(
        "bank robbery",
        message.author.avatarURL()
    );
