...
async function run(message: Message | (NypsiCommandInteraction & CommandInteraction), args: string[]) {
    ...
    if (response.customId == "y") {
        ...
        if (winner == message.member) {
            thingy = `**${message.author.username}** +$${winnings.toLocaleString()}${tax ? ` (${(tax * 100).toFixed(1)}% tax)` : ""
                }\n${target.user.username}`;
        } else {
            thingy = `${message.author.username}\n**${target.user.username}** +$${winnings.toLocaleString()}${tax ? ` (${(tax * 100).toFixed(1)}% tax)` : ""
                }`;
        }
