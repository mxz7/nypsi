...
async function run(
  ...
  if (!message.member.permissions.has(PermissionFlagsBits.ManageGuild)) {
    if (message.member.permissions.has(PermissionFlagsBits.ManageMessages)) {
      send({ embeds: [new ErrorEmbed("you need the `manage server` permission")] });
    }
    return;
  }
  ...
  if (args.length == 0) {
    ...
    if (countdowns.length >= max) {
      ...
      return send({ embeds: [new ErrorEmbed(error)] });
    }
    ...
    let res: any = await message.channel
      ...
      .catch(() => {
        fail = true;
        send({ embeds: [new ErrorEmbed("you ran out of time - cancelled")] });
      });
    ...
    if (date.getTime() - now < 172800000) {
      return send({ embeds: [new ErrorEmbed("thats less than 2 days away bro")] });
    }
    ...
    if (!res.includes("%days%")) {
      return send({ embeds: [new ErrorEmbed("could not find %days%")] });
    }
