...
async function run(
  ...
  const gambleStats = async () => {
    ...
    const options = new StringSelectMenuBuilder().setCustomId("select-game-type");
      ...
      const rows = [
        ...
        new ActionRowBuilder<MessageActionRowComponentBuilder>().addComponents(
          new ButtonBuilder()
            ...
            .setLabel("history")
            .setURL(`https://nypsi.xyz/game?user=${message.author.id}&game=${type}&ref=bot-wordle`)
            .setEmoji("📖"),
        ),
      ];
    ...
    const listen = async () => {
      ...
      if (interaction.customId === "select-game-type") {
