...
export default {
  ...
  async run(interaction) {
    ...
    if (compact.includes("0") && compact !== "0") {
      return res.reply({
    ...
    const digits =
      compact === "0" ? [0] : Array.from(new Set(compact.split("").map((d) => parseInt(d, 10))));
    let workingGame = freshGame;

    for (const coordinate of coordinates) {
      for (const digit of digits) {
        const result = await toggleNote(workingGame, coordinate, digit, coordMode);
