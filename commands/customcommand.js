...
async function run(message, args) {
    if (!isPremium(message.author.id)) {
        return message.channel.send(
            new ErrorEmbed("you must be at least GOLD tier for this command")
        )
    }
    ...
    if (args.length == 0) {
        ...
        if (cmd) {
            if (cmd.content) {
                ...
                embed.addField("uses", cmd.uses, true)
            } else {
        ...
        await message.channel.send(
            new CustomEmbed(message.member, false, "please enter your command name / trigger")
        )
